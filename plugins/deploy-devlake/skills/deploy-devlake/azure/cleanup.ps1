...
.PARAMETER StateFile
    Path to state file (default: .devlake-azure.json in current directory)
...
if (-not (Test-Path $StateFile)) {
    Write-Error "State file not found at: $StateFile`nExpected: .devlake-azure.json in current directory`nOr specify with: -StateFile <path>"
    exit 1
}
